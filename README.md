...
- **Apple System Profiler does not list a USB 2.0 bus, even when the driver is working perfectly.** ASP shows only the buses your machine shipped with (usually `USB 0` and `USB 1`) and lists devices under those. The Hi-Speed controller this driver adds is not in that list, whether or not anything is plugged into it, and a Hi-Speed drive attached to it does not appear as a device anywhere in ASP either. What **does** appear is the mounted **volume**, near the end of the report in the volumes section, under a plain `USB` heading beside `Network`. So if you are using ASP to check your install, look for the **mounted volume, not for a bus**. Measured on a FireWire 800 MDD: a 3.74 GB Hi-Speed flash drive mounted and worked normally while ASP showed no bus for it and no device entry on either original bus. Nothing is wrong; ASP simply cannot see this controller.
